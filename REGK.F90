...
# include "perfmacro.h"
  ...
  use diag,       only: proc0_reads_to_all, flows, b_field, predatasave, &
...
!  real:: elapsed_time
  ...
  real::aa0,dti,x1,x2,x3,ek_u,ek_b, ek_e, hk_a, hk_t, ik_t, ik_a, ek_n, hk_aa, hk_ta, ik_ta, ik_aa,hk_1,hk_2, sumleftover !,new_dti !,N_aa0
...
!     if(turb .and. t==0) then
        !choosing kz
        fac=Lz/2./pi
        kfz=nint(kfz1*fac-.5d0+uniran()*(fac*kfz2-fac*kfz1+1d0))/fac
        ! random sign
...
!        end if
        ...
        call k_energy(1,savetime,p,akpar,phik,nek,gk,dzgk2,braakpargk2,ek_u,ek_b,ek_e,hk_a,hk_t,ik_t,ik_a,ek_n,hk_aa, hk_ta, ik_ta, ik_aa,hk_1,hk_2,runname)
