	       ...
	       do i=1,nkx_par
	          do j=1,nky
	             fapar(j,i,k)=1.0/(1.0+kperp(j,i)**2*de**2)*&
	                  ...
	                  -notanj*1.0/sqrt(2.0)*rhos_de*rhoe_LTe*exp(-(((ky(j)**2+kx(i)**2)**(1./2)/kc0)**4))*(0.0,1.0)*ky(j)*Akpar(j,i,k))
	          end do
	       end do
	    end do
	    else 
	       ...
	       do i = 1, nkx_par
	          do j = 1, nky
	             fg2(j,i,k) = brag2phik(j,i,k) + sqrt(gmin+1.0) * rhos_de * braakparg3(j,i,k) &
	                  ...
	                  - notanj * 1.0/(2.0*rhos*de) *rhoe_LTe*exp(-(((ky(j)**2+kx(i)**2)**(1./2))/kc0)**4)*(0.0,1.0) * ky(j) * Phik(j,i,k)
	              end do
	       end do
	    end do         
	else 
	       ...
	       do i = 1, nkx_par
	          do j = 1, nky
	             fgm(j, i, k) = &
	                  ...
	                  * rhoe_LTe*exp(-(((ky(j)**2+kx(i)**2)**(1./2))/kc0)**4)*ky(j)*Akpar(j, i, k)
	          end do
	       end do
	    end do
	else
