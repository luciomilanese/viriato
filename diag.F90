...
#define WITH_HAC_USE_TIMESTR 1
  ...
  public :: proc0_reads_to_all, flows, b_field, injected
  public :: dataloadtest
#if WITH_HAC
  public :: dataloadtest_hac, datasavetest_hac
#endif
  public :: predatasave, datasavetest, kdatasave3d, test_of_test, cuts, k_energy, gamma_ky
...
! WITH_HAC
     if (g_inc) then
       ...
       call proc0_reads_to_all(restart_file1,&
        ...
        open (unit=13,file=trim(restart_file1),status='OLD')
        do n=0,NPE*npez-1
           if (iproc==n) then
              do k=1,iproc*nly_par*nlz_par
                 do i=1,nlx
                    read(13,*) x1,x2,x3
                 end do
              end do
...
!*********************************************
  subroutine k_energy(unitnumber,time,p,akpar,fik,nek,gk,dzgk2,braakpargk2,ek_u,ek_b,ek_e,hk_a,hk_t,ik_t,ik_a,ek_n,hk_aa,hk_ta,ik_ta,ik_aa,hk_1,hk_2,runname)
    !energy per unit volume
    ...
    complex, dimension(nky,nkx_par,nlz_par,gmin:ngtot)::gk
    complex,dimension(nky,nkx)::sutot,sbtot,setot,shatot,shttot,sittot,siatot,sntot,shaatot,shtatot,sitatot,siaatot,shk1tot, shk2tot
    complex, dimension (nky*nkx_par*NPE) :: work_u,work_b, work_e, work_ha, work_ht, work_it, work_ia, work_n, work_haa, work_hta, work_ita, work_iaa, work_hk1, work_hk2 
    real, dimension(nky,nkx_par)::energy_u,energy_b, energy_e,energy_ha, energy_ht, energy_it, energy_ia, energy_n, energy_haa, energy_hta, energy_ita, energy_iaa, energy_hk1, energy_hk2 ! LUIS, 28/4/15: kinetic, magnetic, electric energy
    real, dimension(:),allocatable::kshell_u,kshell_b,kshell_e,kshell_ha, kshell_ht, kshell_it, kshell_ia, kshell_n,kshell_haa, kshell_hta, kshell_ita, kshell_iaa, kshell_hk1, kshell_hk2
    real:: ek_u,ek_b,ek_e,hk_a,hk_t,ik_t,ik_a,ek_n,hk_aa,hk_ta,ik_ta,ik_aa,hk_1,hk_2,time,kpmax!,de,rhos
...
!       kpmax=sqrt((nky/2.*lx/ly)**2.+((nkx-1.)*1.0)**2.)
       ...
       open (unit=unitnumber, file= trim(filename1))
       write(unitnumber, '(15a15)') 'kglobal(k)', 'kp' ,'kinetic', 'magnetic', 'electric', 'helicity_t', 'helicity_a', 'injection_t', 'injection_a', 'helicity_ta', 'helicity_aa', 'injection_ta', 'injection_aa', 'helicity_real', 'helicity_abs'
     end if
    ...
    do k=1,nlz_par
       ...
       if(mod(iproc,npe)==0) then
          do j=1, nky
             ...
             energy_hk1(j,1)= real((1+de**2*kperp(j,1)**2)*(fik(j,1,k)*conjg(akpar(j,1,k))))
             energy_hk2(j,1)= abs((1+de**2*kperp(j,1)**2)*(fik(j,1,k)*conjg(akpar(j,1,k))))    
          ...
          do i=2,NKx_par
             do j=1,NKy
                ...
                energy_hk1(j,i)= 2.0*real((1+de**2*kperp(j,i)**2)*(fik(j,i,k)*conjg(akpar(j,i,k))))
                energy_hk2(j,i)= 2.0*abs((1+de**2*kperp(j,i)**2)*(fik(j,i,k)*conjg(akpar(j,i,k))))
       ...
       If(p .GT. 1  .AND. mod(p,save_energyfiles)==0)THEN
          ...
          if(proc0) then
             ...
             do kp=1,ceiling(kpmax)
                ...
                write(unitnumber, '(2i5,14g16.8)') kglobal(k), kp ,kshell_u(kp)/(nlz*1.), &
				&kshell_b(kp)/(nlz*1.), kshell_e(kp)/(nlz*1.), kshell_ht(kp)/(nlz*1.), kshell_ha(kp)/(nlz*1.), kshell_it(kp)/(nlz*1.0), kshell_ia(kp)/(nlz*1.0), kshell_hta(kp)/(nlz*1.), kshell_haa(kp)/(nlz*1.), kshell_ita(kp)/(nlz*1.0), kshell_iaa(kp)/(nlz*1.0), kshell_n(kp)/(nlz*1.0), kshell_hk1(kp)/(nlz*1.0), kshell_hk2(kp)/(nlz*1.0)
             end do
